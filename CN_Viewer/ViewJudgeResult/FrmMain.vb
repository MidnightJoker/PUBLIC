...
#End Region
    ...
    Private Sub objJudgeResultFile(ByRef objJRdata As ClsJRdata, ByVal teeNode As TreeNode)
        ...
        For i = j To strLine.Length - 1
            ...
            End If
            funcName = strElement(IdxType).Trim
            ...
            If Not (func.Type = defectType.NULL Or func.Type = defectType.POINT_COUNT Or func.Type = defectType.MURA) Then
                teeNode.Nodes.Add(CheckDefectName(strElement(IdxType).Trim) & "(" & func.Data & "," & func.Gate & ")")
        ...
        For i = j To strLine.Length - 1
            ...
            teeNode.Nodes.Add("Mura" & "(" & mura.Data & "," & mura.Gate & ")" & " = " & CheckDefectName(mura.MuraType.Trim) & "[" & mura.Pattern & "]")
            objJRdata.AddMura(mura)
            teeNode.Nodes.Item(teeNode.Nodes.Count - 1).ToolTipText = "(" & mura.Data & "," & mura.Gate & ")" & "; " & CheckDefectName(mura.MuraType.Trim) & "; " & mura.MaxX & "," & mura.MaxY & "," & mura.MinX & "," & mura.MinY & "; " & mura.ImageFilePath & mura.FileName
        Next
    End Sub
    ...
    Private Sub view_DefectWinUpper(Optional ByVal intMarkX As Integer = 0, Optional ByVal intMarkY As Integer = 0) 'CCDMODE Select
        Dim mousepos As New Point(intMarkX, intMarkY)
        For Index As Integer = 0 To _M_DefectFrmPositionList.Count - 1
            If _M_DefectFrmPositionList(Index) = mousepos Then
                M_DlgShowDefectFrm(Index).BringToFront()
                Exit For
            End If
        Next
        ...
        For Index As Integer = 0 To _F_DefectFrmPositionList.Count - 1
            If _F_DefectFrmPositionList(Index) = mousepos Then
                F_DlgShowDefectFrm(Index).BringToFront()
                Exit For
            End If
        Next
    ...
    Private Function CheckDefectTextPosition(ByVal PositionX As Integer, ByVal PositionY As Integer,
        ...
        Dim SmallWindow_LT As Point
        Dim SmallWindow_RB As Point
        SmallWindow_LT = New Point(PositionX - 50, PositionY + 10)
        SmallWindow_RB = New Point(PositionX + 100 - 50, PositionY + 46 + 10)

        If SmallWindow_LT.X < 0 Then
            SmallWindow_LT.X = PositionX + 10
            SmallWindow_RB.X = PositionX + 100 + 10
        End If

        If SmallWindow_RB.X > PicboxW Then
            SmallWindow_LT.X = PositionX - 100 - 10
            SmallWindow_RB.X = PositionX - 100 - 10
        End If

        If SmallWindow_RB.Y > PicboxH Then
            SmallWindow_LT.Y = PositionY - 46 - 10
            SmallWindow_RB.Y = PositionY - 46 - 10
        End If
    ...
    Private Function CheckDefectName(ByVal EnDefectName As String)
        ...
        Select Case (EnDefectName)
            Case "BP", "BP-Pair", "BP-Near", "BP-Cluster", "3BP-ADJ"
                CNDefectName = "亮点"
            Case "DP", "Group-Small-DP", "DP-Pair", "DP-Near"
                CNDefectName = "暗点"
            Case "H-Line", "V-Line"
                CNDefectName = "线缺陷"
            Case "Black-Spot"
            ...
            Case "White-Spot"
                CNDefectName = "白团"
            Case "V-Band-Mura", "White-V-Band", "Black-V-Band"
                CNDefectName = "垂直Mura"
            Case "H-Band-Mura", "White-H-Band", "Black-H-Band"
                CNDefectName = "水平Mura"
            Case "GSBP", "Group-Small-BP"
                CNDefectName = "红蓝班"
        End Select
        Return CNDefectName
    End Function
    Private Function CheckDefectColor(ByVal EnDefectName As String)
        Dim DefectColor As Color

        Select Case (EnDefectName)
            Case "BP", "BP-Pair", "BP-Near", "BP-Cluster", "3BP-ADJ"
                DefectColor = System.Drawing.Color.FromArgb(Me.m_Config.colorBP1)
            Case "DP", "Group-Small-DP", "DP-Pair", "DP-Near"
                DefectColor = System.Drawing.Color.FromArgb(Me.m_Config.colorDP1)
            Case "H-Line", "V-Line"
                DefectColor = System.Drawing.Color.FromArgb(Me.m_Config.colorXL)
            Case "Black-Spot"
                DefectColor = System.Drawing.Color.FromArgb(Me.m_Config.colorDP1)
            Case "White-Spot"
                DefectColor = System.Drawing.Color.FromArgb(Me.m_Config.colorBP1)
            Case "V-Band-Mura", "White-V-Band", "Black-V-Band"
                DefectColor = System.Drawing.Color.FromArgb(Me.m_Config.colorVL)
            Case "H-Band-Mura", "White-H-Band", "Black-H-Band"
                DefectColor = System.Drawing.Color.FromArgb(Me.m_Config.colorHL)
            Case "GSBP", "Group-Small-BP"
                DefectColor = System.Drawing.Color.FromArgb(Me.m_Config.colorGSBP)
        End Select
        Return DefectColor
    End Function
            ...
            For i = 0 To JRdataList.MuraCount - 1
                ...
                If mura.Data > 0 AndAlso mura.Gate = -1 Then 'V-Band
                    ...
                    ElseIf rbNormalView.Checked = True AndAlso mura.Pattern.IndexOf("CENTER") = -1 Then
                        ...
                        gr.DrawLine(Me.penVL, CInt(mura.Data * sx), 0, CInt(mura.Data * sx), CInt(H - 1.0))
                        'Continue For
                ...
                ElseIf mura.Data = -1 AndAlso mura.Gate > 0 Then 'H-Band
                    ...
                    ElseIf rbNormalView.Checked = True AndAlso mura.Pattern.IndexOf("CENTER") = -1 Then
                        Me.penMURA.Width = 1.0F
                        If Me.m_Config.bMuraCcdNo Then gr.DrawString(CStr(mura.CCDNo), Me.Font, Brushes.White, CInt(data * sx), CInt(gate * sy))
                        gr.DrawLine(Me.penHL, 0, CInt(mura.Gate * sy), CInt(W - 1.0), CInt(mura.Gate * sy))
                        'Continue For
                    ElseIf rbRightView.Checked = True AndAlso mura.Pattern.IndexOf("RIGHT") = -1 Then
                        Continue For
                    Else
...
CheckDefectName(mura.MuraType.Trim), mura.Pattern)

                    Else
                        M_DlgShowDefectFrm(i).NoImage(CheckDefectName(mura.MuraType.Trim), mura.Pattern, CheckDefectColor(mura.MuraType.Trim))
                    End If
                    If Not Me.m_Config.bUseDefectDynamicShowWin Then
...
CheckDefectName(func.Type.ToString().Trim), func.Pattern)

                    Else
                        F_DlgShowDefectFrm(i).NoImage(CheckDefectName(func.FuncName), func.Pattern, CheckDefectColor(func.FuncName))
                    End If
                    If Not Me.m_Config.bUseDefectDynamicShowWin Then
    ...
    Private Sub teeView_AfterSelect(ByVal sender As System.Object, ByVal e As System.Windows.Forms.TreeViewEventArgs) Handles teeView.AfterSelect
        ...
        Else
            Try
                ...
                Else
                    Me.view_DefectWinUpper(Dfx / sx, Dfy / sy)
                End If
            Catch ex As Exception
                'MessageBox.Show(ex.StackTrace, "ViewJudgeResult Error")
            End Try
...
#Region "宮格/視角設定"
    ...
    Private Sub rbNormalView_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbNormalView.CheckedChanged
        If rbNormalView.Checked = True Then
            ...
            If Me.m_Config IsNot Nothing Then
                If Not Me.m_Config.bUseDefectDynamicShowWin Then
                    For Index As Integer = 0 To _M_DefectFrmPositionList.Count - 1
                        M_DlgShowDefectFrm(Index).Show()
                    Next
